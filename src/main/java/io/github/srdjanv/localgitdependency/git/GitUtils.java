...
public final class GitUtils {
    ...
    public static Git cloneRepo(GitInfo gitInfo) throws GitAPIException {
        ...
        Git git = Git.cloneRepository()
                ...
                .setBare(false)
                .setNoCheckout(true)
                .setCloneSubmodules(gitInfo.isCloneGitSubmodules())
                .call();
    ...
    public static void checkout(GitRepo repo) throws GitAPIException {
        repo.getGit()
                .checkout()
                .setCreateBranch(true)
                .setName(repo.getRemoteBranchName())
                .setStartPoint(repo.getGitInfo().getTargetRemote())
                .call();
    }

    public static void update(GitRepo repo) throws GitAPIException, IOException {
        final long start = System.currentTimeMillis();
        final String localTargetBranch =
                switch (repo.getGitInfo().getTargetType()) {
        ...
        repo.getGitInfo().setRefreshed();
        ManagerLogger.info(
                "Finished updating {} to {} in {} ms",
                repo.getGitInfo().getUrl(),
                repo.getGitInfo().getTarget(),
    ...
    public static String getCorrespondingRemoteBranchNameFromCurrentTarget(GitRepo repo)
            throws GitAPIException, IOException {
        final String remoteBranch;
        if (repo.getGitInfo().getTargetType() == BRANCH) {
            remoteBranch = repo.getGitInfo().getTarget();
        } else {
            final ObjectId objectid;
            if (repo.getGitInfo().getTargetType() == GitInfo.TargetType.TAG) {
                objectid = repo.getGit()
                        .getRepository()
                        .exactRef(repo.getGitInfo().getTargetRemote())
                        .getObjectId();
            } else { // commit
                objectid = ObjectId.fromString(repo.getGitInfo().getTargetRemote());
            }
            ...
            String branch = optional.orElseThrow(() -> new NoSuchElementException(String.format(
                    "Target: %s not found in remote", repo.getGitInfo().getTarget())));
            if (branch.contains("~")) {
    ...
    public static boolean hasBranchLocalCommits(GitRepo repo) throws IOException {
        if (repo.getGitInfo().getTargetType() == BRANCH) {
            Ref remoteBranch = repo.getGit()
                    .getRepository()
                    .getRefDatabase()
                    .exactRef(repo.getGitInfo().getTargetRemote());
            Ref localBranch = repo.getGit()
                    .getRepository()
                    .getRefDatabase()
                    .exactRef(repo.getGitInfo().getTargetLocal());
    ...
    public static boolean isUpToDateWithRemote(GitRepo repo) throws IOException {
        final String headId = repo.getHeadID();
        final String target = repo.getGitInfo().getTargetRemote();
        switch (repo.getGitInfo().getTargetType()) {
            case COMMIT -> {
            ...
            case BRANCH -> {
                ...
                throw new RuntimeException(String.format(
                        "Unable to locate remote branch: '%s", repo.getGitInfo().getTarget()));
            }
