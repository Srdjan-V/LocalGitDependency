...
public final class LGDIDE extends GroovyObjectSupport {
    ...
    private final NamedDomainObjectContainer<DefaultSourceSetMapper> mappers;
    ...
    public LGDIDE(final Managers managers) {
        ...
        this.mappers = managers.getProject().getObjects().domainObjectContainer(DefaultSourceSetMapper.class, name -> {
            return new DefaultSourceSetMapper(container, managers, name);
        });
    }

    public void mappers(Action<NamedDomainObjectContainer<DefaultSourceSetMapper>> action) {
        action.execute(mappers);
    }

    public NamedDomainObjectContainer<DefaultSourceSetMapper> getMappers() {
        return mappers;
    }
